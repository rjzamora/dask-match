...
@pytest.mark.parametrize(
    ...
    [
        ...
        xfail_param_gpu(lambda df: df.mask(df.x == 10), "cudf np.nan support"),
        xfail_param_gpu(lambda df: df.mask(lambda df: df.x % 2 == 0, 42)),
        lambda df: df.mask(df.x == 10, df + 2),
        xfail_param_gpu(lambda df: df.mask(df.x == 10, lambda df: df + 2)),
        lambda df: df.x.mask(df.x == 10, 42),
        lambda df: df.abs(),
        lambda df: df.x.abs(),
        lambda df: df.where(df.x == 10, 42),
        xfail_param_gpu(lambda df: df.where(df.x == 10), "cudf np.nan support"),
        xfail_param_gpu(lambda df: df.where(lambda df: df.x % 2 == 0, 42)),
        lambda df: df.where(df.x == 10, df + 2),
        xfail_param_gpu(lambda df: df.where(df.x == 10, lambda df: df + 2)),
        ...
        xfail_param_gpu(lambda df: df.x.rename(index=df.x)),
        lambda df: df.x.rename(index=("hello",)),
